...
| Sep 2026 | Voucher diskon (%) di checkout + admin CRUD voucher | Voucher `discount_type='percent'` dengan `discount_value` (1–100), scope `applies_to` (`all`/`plan`/`package`) + `plan_types`/`package_ids`. Validasi terpusat di `lib/voucher.ts`; harga dihitung ulang server-side di `/api/payment/create` (client tidak dipercaya). Input kode ada di modal `CheckoutModal` SEBELUM Snap dibuka (popup Midtrans tidak bisa dikustomisasi). Voucher ditandai terpakai saat webhook `paid` (1x per user via `voucher_uses`). Audit di `subscriptions`: `voucher_code`, `original_amount`, `discount_amount`. Migrasi: `2026-09-12_voucher_discount.sql`. |
| Sep 2026 | Nomor percobaan `attempts.attempt_number` + view `admin_user_stats` | Nomor percobaan per user+paket disimpan sebagai kolom (diisi trigger `set_attempt_number` saat insert, backfill untuk data lama) agar bisa difilter/diurutkan tanpa batas 1000 baris PostgREST. View `admin_user_stats` (service-role only) menyediakan `attempt_count` & `total_spending` per user untuk sorting di `/admin/users`. Migrasi: `2026-09-12_attempt_number.sql`, `2026-09-12_admin_user_stats.sql`. |
