...
async function main() {
  ...
  if (!pkgs.length) {
    console.log('Tidak ada paket-2 ditemukan')
    return
  }
  ...
  for (const p of pkgs.sort((a, b) => a.slug.localeCompare(b.slug))) {
    ...
    const stream = p.slug.replace('-paket-2', '').replace('antam-', '')
    const flags = []
    for (const q of qs) {
  ...
  console.log('Stream'.padEnd(12), 'Soal', 'Flagged')
  for (const [s, v] of Object.entries(perStream)) {
    console.log(s.padEnd(12), String(v.total).padEnd(6), v.flagged)
  }
  console.log(`\nTOTAL paket-2: ${pkgs.length} paket | ${totalQ} soal | ${totalFlag} flagged`)
}
