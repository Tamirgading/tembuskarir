...
export async function POST(req: NextRequest) {
  try {
    ...
    const { id, displayName, score, durationSeconds } = body as {
      id?: string
      displayName?: string
      score?: number
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Gagal memperbarui entri leaderboard.' }, { status: 500 })
    }

    return NextResponse.json({ message: 'Entri leaderboard berhasil diperbarui.' })
  } catch (err) {
