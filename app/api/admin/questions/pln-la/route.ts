...
export async function POST(req: NextRequest) {
  ...
  const { packageId, content, opt_a, opt_b, opt_c, opt_d, opt_e,
    point_a, point_b, point_c, point_d, point_e, dimension, is_reverse_scored, explanation } = body

  if (!packageId || !content?.trim() || !opt_a || !opt_b || !opt_c || !opt_d || !opt_e || !dimension) {
  ...
  const { count } = await (service.from('questions_pln_la') as any)
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data, error } = await (service.from('questions_pln_la') as any)
    .insert({
      ...
      dimension,
      is_reverse_scored: !!is_reverse_scored,
  ...
  if (error) {
    console.error('[pln-la] insert error:', error)
    return NextResponse.json({ error: 'Gagal menyimpan soal LA.' }, { status: 500 })
  }
